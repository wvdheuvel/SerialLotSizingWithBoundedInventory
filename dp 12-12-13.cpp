...
{	double **cumD, ******cost, tmpCst, minCst, eps=0.00001, inf=pow(10.0,10.0);	
	...
	int tmpI;
	SetDim2Dbl(cumD,n,n);
	SetDim6Dbl(cost,L+2,n,n,n,n,n);
	if (checkUsed)
		SetConstDim6Dbl(cost,L+2,n,n,n,n,n,-1.0);
	cumDemands(n,d,cumD);
	...
	{	for (int t1=0; t1<=t2; t1++)
		{	for (int j=0; j<=t1; j++)
			{	cost[L+1][j][j][j][t1][t2]=K[L+1][j]+p[L+1][j]*cumD[t1][t2]+costZW[2][j][t1][t2];
				//cost[L+1][j][j][j][t1][t2]=costBN[0][j][t1][t2];
		...
		{	for (int b1=0; b1<=t1; b1++)
			{	for (int b2=b1+1; b2<=t2; b2++)
				{	minCst=inf;
					...
					if ((checkUsed) && (cost[L+1][b1][b1][b2-1][t1][t2]<-eps))
					...
					cost[L+1][b1][b1][b2][t1][t2]=minCst;
				}	
			}
		}
	}
				...
				{	for (int t1=b1;t1<n;t1++)	
					{	for (int t2=maxI(t1,b2);t2<n;t2++)
						{	
							...
							// special case: production flow only 
							if ((i<L)||(j==b1))
							{	tmpCst=K[i][j]+p[i][j]*cumD[t1][t2]+cost[i+1][j][b1][b2][t1][t2];
								if (tmpCst<minCst)
									minCst=tmpCst;
								if ((checkUsed) && (cost[i+1][j][b1][b2][t1][t2]<-eps))
							...
							{	if (i==0) //dummy level
									tmpCst=cost[i][j+1][b1][b2][t1][t2];
								else
									tmpCst=h[i-1][j]*cumD[t1][t2]+cost[i][j+1][b1][b2][t1][t2];

								if ((checkUsed) && (cost[i][j+1][b1][b2][t1][t2]<-eps))
							...
							{	for (int b=b1; b<=b2-1; b++)								
								{	for (int t=maxI(b,t1); t<=t2-1; t++)
									{	tmpCst=K[i][j]+p[i][j]*cumD[t1][t]+cost[i+1][j][b1][b][t1][t];
										if ((checkUsed) && (cost[i+1][j][b1][b][t1][t]<-eps))
										...
										if (i==0) //dummy level
											tmpCst+=cost[i][j+1][b+1][b2][t+1][t2];
										else
											tmpCst+=h[i-1][j]*cumD[t+1][t2]+cost[i][j+1][b+1][b2][t+1][t2];

										if ((checkUsed) && (cost[i][j+1][b+1][b2][t+1][t2]<-eps))
	...
	{	for (int t1=b1; t1<n; t1++)
		{	for (int t2=t1; t2<n; t2++)
			{	for (int b2=b1; b2<=t2; b2++)
				{	minCst=cost[0][0][b1][b2][t1][t2];
					if ((checkUsed) && (cost[0][0][b1][b2][t1][t2]<-eps))
					{	cout << "Error: var not set US 09" << endl;
						cin >> tmpI;
						tmpI=tmpI;
					}
					for (int j=1; j<=b1; j++)
					{	tmpCst=cost[0][j][b1][b2][t1][t2];
						if ((checkUsed) && (cost[0][j][b1][b2][t1][t2]<-eps))
	...
	DeleteDim6Dbl(cost,L+2,n,n,n,n,n);
	DeleteDim2Dbl(cumD,n,n);
}
