...
void SetConstDim6Int(int ******x, int n1, int n2, int n3, int n4, int n5, int n6, int c);
	...
	{	for (int b1=0; b1<t2; b1++)
		{	for (int t1=b1; t1<=t2; t1++)
			{	/*if ((b1==2) && (t1==3) && (t2==4))
				...
				if ((cumD[t1][t2]>U+eps)&&((m>=3)||(t1==b1))) // case m=2 is special case
					OptDownstreamPath(m,n,d,K,p,h,U,b1,t1,t2,costZW,costBN,costDS);
				else
...
{	double inf=pow(10.0,10.0), eps=0.00001, tmpCst, minCst;
	...
	for (int j=maxI(b1+1,t1); j<=tUB; j++) 
	{	minCst=inf;
	...
	{	minCst=inf;
		...
		//PrintCost(b1,b2,t1,t2,costDS[b1][b2][t1][t2],"DS");
	}
...
void UpstreamTrees(int L, int n, double *d, double **K, double **p, double **h, double ****costZW, double ****costBN, double ****costDS, double ****costUS)
{	double **cumD, ******cost, tmpCst, minCst, minCst2, eps=0.00001, inf=pow(10.0,10.0);	
	// 26-07-2016
	int ******argmin, ******argmax, b1s, b2s;
	//
	bool checkUsed=false;	// checks whether all variables are set when used
	bool checkArg;			// checks whether argmin and argmax get assigned the right values
	bool k=0;				// only use two SC levels in implemtation of DP
	bool useImprove=true;	// 1 if enhanced DP is used in case of NS motives at downstream level
	int tmpI;
	SetDim2Dbl(cumD,n,n);
	...
	//26-07-16
	SetDim6Int(argmin,2,n,n,n,n,n);
	SetDim6Int(argmax,2,n,n,n,n,n);
	SetConstDim6Int(argmin,2,n,n,n,n,n,-1);
	SetConstDim6Int(argmax,2,n,n,n,n,n,-1);
	//
	...
	{	for (int t1=0; t1<=t2; t1++)
		{	for (int j=0; j<=t1; j++)
			{	cost[0][j][j][j][t1][t2]=K[L+1][j]+p[L+1][j]*cumD[t1][t2]+costZW[2][j][t1][t2];
				// 26-07-16
				argmin[0][j][j][j][t1][t2]=j;
				argmax[0][j][j][j][t1][t2]=j;
				//
		...
		{	for (int b1=0; b1<=t1; b1++)
			{	for (int b2=b1+1; b2<=t2; b2++)
				{	minCst=inf;
					...
					if (useImprove) 
					{	for (int b=b2; b>=b1; b--)
						{	if (abs(cost[0][b1][b1][b][t1][t2]-minCst)<eps)
								argmin[0][b1][b1][b2][t1][t2]=b;						
						}
						for (int b=b1; b<=b2; b++)
						{	if (abs(cost[0][b1][b][b2][t1][t2]-minCst)<eps)
								argmax[0][b1][b1][b2][t1][t2]=b;						
						}					
					}
					//
				}	
	...
	//General DP -> use backward recursion
	for (int i=L;i>=0;i--)
	{	k=1-k; //change row index
		...
		// 26-07-2016
		if (useImprove) 
		{	argmin[k][n-1][n-1][n-1][n-1][n-1]=n-1;
			argmax[k][n-1][n-1][n-1][n-1][n-1]=n-1;
		}
		//
		
		for (int j=n-2;j>=0;j--)
		{	for (int b1=n-1; b1>=j; b1--) // for (int b1=j; b1<n; b1++) //
			{	for (int b2=b1; b2<n; b2++)
				{	for (int t1=b1;t1<n;t1++)	
					{	for (int t2=maxI(t1,b2);t2<n;t2++)
						{	
							//if ((i==1)&&(j==0)&&(b1==0)&&(b2==2)&&(t1==0)&&(t2==2))
							/*if ((i==2)&&(j==0)&&(b1==0)&&(b2==4)&&(t1==0)&&(t2==4))
							{	cout << "Stop for debugging" << endl;
								cin >> tmpI;
								tmpI=tmpI;
							}*/
							...
							// production and inventory flow
							// these loops lead to a LT^7 algorithm
							if ((!useImprove)&&((i<L)||(j==b1)))
							...
							// 26-07-2016
							// production and inventory flow
							if ((useImprove) && (((i<L)||(j==b1))&&(b1<b2)))
							{	for (int t=t1; t<=t2-1; t++)
								{	b1s=argmin[1-k][j][b1][minI(t,b2)][t1][t];
									b2s=argmax[k][j+1][maxI(j+1,b1)][b2][t+1][t2];
									if ((b1s<0)||(b2s<0))
									{	cout << "Error: b1s or b2s not properly set" << endl;
										cin >> tmpCst;
									}

									//if (b1s<b2s) // this case only needed based on Hark-Chin's new theorem									
									// 29-08-2016 'if' is not needed anymore based on new insights
									// still difference with MIP -> maybe b1s and b2s not set correctly
									// 29-08-2016 end
									{	// prod cost branch
										tmpCst=K[i][j]+p[i][j]*cumD[t1][t]+cost[1-k][j][b1][b1s][t1][t];

										// hold cost branch
										if (i==0) //dummy level
											tmpCst+=cost[k][j+1][b2s][b2][t+1][t2];
										else
											tmpCst+=h[i-1][j]*cumD[t+1][t2]+cost[k][j+1][b2s][b2][t+1][t2];										
							...
							/*if (abs(minCst2-minCst)>eps)
							{	cout << "Error: approach to improve LT7 does not work!" << endl;
								cin >> tmpCst;
							}*/
							//
							...
							if (useImprove) // use for new LT6 approach
							{	cost[k][j][b1][b2][t1][t2]=minCst2;
								minCst=minCst2;
							}
							else // use for old LT7 approach
								cost[k][j][b1][b2][t1][t2]=minCst;

							//26-07-16
							// find b1start and b2start as in Hark-Chins 3 page file
							if (useImprove)
							{	for (int b=b2; b>=b1; b--)
								{	checkArg=false;
									if (abs(cost[k][j][b1][b][t1][t2]-minCst)<eps)									
									{	checkArg=true;
										argmin[k][j][b1][b2][t1][t2]=b;
									}
									if (checkArg=false)
									{	cout << "Error: in setting argmin" << endl;
										cin >> tmpCst;
									}
								}
								for (int b=b1; b<=b2; b++)
								{	checkArg=false;
									//if (abs(cost[k][j][b][b2][t1][t2]-minCst)<eps)
									tmpCst=cost[k][j][b][b2][t1][t2];
									if ((tmpCst>=-eps)&&(tmpCst<=minCst+eps)) // RN with later b's may be better
									{	checkArg=true;
										argmax[k][j][b1][b2][t1][t2]=b;
										minCst=tmpCst;
									}
									if (checkArg=false)
									{	cout << "Error: in setting argmax" << endl;
										cin >> tmpCst;
									}
								}
							}
	...
	DeleteDim6Int(argmin,2,n,n,n,n,n);
	DeleteDim6Int(argmax,2,n,n,n,n,n);
	DeleteDim2Dbl(cumD,n,n);
}
