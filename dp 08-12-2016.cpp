...
{	double **cumD, ******cost, tmpCst, minCst, minCst2, eps=0.00001, inf=pow(10.0,10.0);	
	...
	bool useImprove;		// 1 if enhanced DP is used in case of NS motives at downstream level
	int tmpI;
	SetDim2Dbl(cumD,n,n);
				...
				{	for (int t1=b1;t1<n;t1++)	
					{	for (int t2=maxI(t1,b2);t2<n;t2++)
						{	
							...
							// use for old LT7 approach
							//cost[k][j][b1][b2][t1][t2]=minCst;

							// use following lines together for new LT6 approach
