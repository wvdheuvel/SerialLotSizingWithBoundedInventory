...
{	double inf=pow(10.0,10.0), eps=0.00001, tmpCst, minCst;
	...
	/*for (int t=b1; t<=t1-1; t++) // removed 27-11-13
		costDSP[1][b1][0][t]=inf;
	for (int t=t1+1; t<=tUB+1; t++)
		costDSP[1][b1][0][t]=inf;*/
	
	// other type 0 arcs at level 1
	/*for (int j=b1+1; j<=tUB; j++) // removed 27-11-13
	{	for (int t=b1; t<=tUB+1; t++)
			costDSP[1][j][0][t]=inf;
	}*/
	...
	// type 0 arcs at period b1 and cum demands from t1 onwards
	/*for (int i=2; i<m; i++) //removed 27-11-13
	{	for (int t=t1+1; t<=tUB+1; t++)
			costDSP[i][b1][0][t]=inf;		
	}*/
	...
	{	if (b1<t1)
		...
		/*else //removed 17-11-13
			costDSP[i][b1][1][t1]=inf;*/
	...
	/*for (int t=t1; t<=tUB+1; t++) //removed 27-11-13
		costDSP[m-1][b1][1][t]=inf;*/
	if (b1==t1)
	...
	/*else //removed 27-11-13
	{	for (int t=b1; t<=t2+1; t++)
			costDSP[m-1][b1][1][t]=inf;
	}*/
	...
	//type 1 arcs 
	/*for (int j=b1+1; j<=t1-1; j++) 
	{	costDSP[m-1][j][1][j+1]=inf;		
	}*/
	...
	for (int i=m-2; i>=1; i--)
	{	//for (int t=b1; t<=tUB+1; t++) // tUB changed 26-11-13
		//	costDSP[i][tUB+1][2][t]=inf;
